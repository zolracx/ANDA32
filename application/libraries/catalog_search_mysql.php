...
class Catalog_search{    	
	...
	function search($limit=15, $offset=0)
	{
		...
		else 
		{
			...
			{
				//$this->ci->db->order_by($sort['sort_by'],$sort['sort_order']);
				//$this->ci->db->order_by('RIGHT(ltrim(rtrim(titl)), 4) desc,  substring(titl, 1,31),  abbreviation');
				
				//print_r($sort['sort_by'] . ' ----> ' . $sort['sort_order'] . '<BR />');
				
				
				if ($sort['sort_by']=='titl'){
					$order_titl = 	$sort['sort_order'];
				}
				
				if ($sort['sort_by']=='proddate'){
					$order_proddate = 	$sort['sort_order'];
				}
			}
			


			//echo $order_titl . ' ---> ' . $order_proddate;
			
			if($order_titl=='desc' && $order_proddate=='desc'){
				echo $order_titl . ' >> ' . $order_proddate;
				$this->ci->db->order_by('substring(titl, 1,31) desc,  RIGHT(ltrim(rtrim(titl)), 4) desc,  abbreviation desc');
			
			}else if($order_titl=='asc' && $order_proddate=='asc'){
				$this->ci->db->order_by('RIGHT(ltrim(rtrim(titl)), 4) ,  substring(titl, 1,31) ,  abbreviation desc');
				
			}else if($order_titl=='asc' && $order_proddate=='desc'){
				$this->ci->db->order_by('RIGHT(ltrim(rtrim(titl)), 4) desc ,  substring(titl, 1,31) ,  abbreviation desc');
